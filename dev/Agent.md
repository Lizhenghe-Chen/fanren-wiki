...
- **v38：页面「最后更新」改为部署时自动注入，不再手写（2026-09-20）**——用户指令「主页是否有可能直接读取 GitHub 更新时间或者一些其它信息，这样就不用手动写更新时间了」。
  ...
  ⑩ **移动端补信息（同日）**：窄屏（≤1100px）原本把顶栏状态位整块 `display:none`，手机上看不到「更新日期 / 作者主页」（用户反馈"移动端有些信息似乎没显示"）。改为只隐藏前缀（`持续更新中 · 作者主页 ·` 包进 `.tn-brief`），`更新 YYYY-MM-DD` 任何宽度都显示；作者主页入口下沉到页脚。⚠️ **顶栏结构一变就要同步锚点**：`stamp.mjs` 的导航栏锚点现为 `</span>更新 `（原来是 `· 更新 `），`verify.mjs` 的导航栏正则同步 —— 两处必须一起改，否则部署会失败（这是设计上的提醒，不是缺陷）。

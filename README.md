...
19 条断言，每条只查一件事：七个视图容器与各自的唯一 h1 / 四库卡片数（221/40/64/65）/ 互引网络规模 / 核心圈默认生效且可一键切回全部 / 无横向溢出与无竖条文本（桌面 1440×900 与移动 390×844 各跑一遍）/ 篇章筛选 / 搜索定位落点 / 卡片互引跳转 / 返回顶部 / 深链冷启动 / 点卡片同步 URL / 浏览器返回后视图与地址一致 / 无第三方资源请求 / 控制台无报错。失败会打印实测值并以非 0 退出码结束。
